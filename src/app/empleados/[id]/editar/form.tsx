...
import { updateEmpleadoSchema, ROLES, AREAS, type UpdateEmpleadoFormData } from "@/shared/validation"
...
export function EditarEmpleadoForm({ empleado, onSuccess }: { empleado: EmpleadoData; onSuccess?: () => void }) {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting }, setError } = useForm<UpdateEmpleadoFormData>({
    ...
    defaultValues: {
      ...
      password: "",
      rol: empleado.rol as UpdateEmpleadoFormData["rol"],
      area: empleado.area as UpdateEmpleadoFormData["area"],
    },
  })

  async function onSubmit(data: UpdateEmpleadoFormData) {
    const payload = { ...data }
    ...
    if (!res.ok) {
      ...
      setError("root", { message: json.error || "Error al actualizar empleado" })
      return
    }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="space-y-2">
        <label htmlFor="password" className="text-sm font-medium">Contraseña <span className="text-muted-foreground">(dejar vacío para no cambiar)</span></label>
